...
st.markdown(
   ...
   <style>
        ...
        /* === ULTRA-STRONG DARK SOURCE CODE DISPLAY (black background) - works on Windows === */
...
with col_code:
    if st.button("Show full source code", use_container_width=True):
        try:
            # Use inspect to get the source of the current running script (works in Streamlit)
            source_code = inspect.getsource(__import__("__main__"))
            st.code(source_code, language="python")
        except Exception as e:
            # Fallback: if inspect fails (rare), show a message
